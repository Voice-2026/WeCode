$ErrorActionPreference = "Stop"
$argv = @($args)
$profileId = if ($argv.Count -gt 0) { [string]$argv[0] } else { "" }
$wrapperDir = if ($PSScriptRoot) { $PSScriptRoot } else { Split-Path -Parent $MyInvocation.MyCommand.Path }
if ($wrapperDir.StartsWith('\\?\UNC\')) { $wrapperDir = '\\' + $wrapperDir.Substring(8) }
elseif ($wrapperDir.StartsWith('\\?\')) { $wrapperDir = $wrapperDir.Substring(4) }
$helper = Join-Path $wrapperDir "wecode-wrapper-helper.exe"
$jsonOutput = "false"

function Usage {
  Write-Host "usage: wecode-db list"
  Write-Host "       wecode-db <profile-id> -- '<statement>'"
  Write-Host "       wecode-db <profile-id> --file <path>"
}

if ($profileId -eq "-h" -or $profileId -eq "--help" -or $profileId -eq "help") {
  Usage
  exit 0
}

if ($profileId -eq "--json") {
  $jsonOutput = "true"
  $argv = @($argv | Select-Object -Skip 1)
  $profileId = if ($argv.Count -gt 0) { [string]$argv[0] } else { "" }
}

$profilesFile = $env:WECODE_DB_PROFILES_FILE
if ([string]::IsNullOrWhiteSpace($profilesFile) -and -not [string]::IsNullOrWhiteSpace($env:DMUX_APP_SUPPORT_ROOT)) {
  $profilesFile = Join-Path $env:DMUX_APP_SUPPORT_ROOT "db_profiles.json"
}
if ([string]::IsNullOrWhiteSpace($profilesFile)) {
  $profilesFile = Join-Path $env:APPDATA "WeCode\db_profiles.json"
}

if ([string]::IsNullOrWhiteSpace($env:WECODE_DB_PROJECT_ID)) {
  [Console]::Error.WriteLine("wecode-db: missing WeCode project context")
  exit 64
}
if (-not (Test-Path $profilesFile)) {
  [Console]::Error.WriteLine("wecode-db: unable to read database profile file")
  exit 66
}
if (-not (Test-Path $helper)) {
  [Console]::Error.WriteLine("wecode-db: bundled helper is missing")
  exit 127
}

if ($profileId -eq "list" -or $profileId -eq "--list" -or $profileId -eq "profiles") {
  $env:WECODE_DB_PROFILES_FILE = $profilesFile
  & $helper --wecode-wrapper-helper db-list-profiles
  exit $LASTEXITCODE
}

if ([string]::IsNullOrWhiteSpace($profileId)) {
  [Console]::Error.WriteLine("wecode-db: missing profile id")
  exit 64
}

$remaining = @($argv | Select-Object -Skip 1)
if ($remaining.Count -gt 0 -and [string]$remaining[0] -eq "--json") {
  $jsonOutput = "true"
  $remaining = @($remaining | Select-Object -Skip 1)
}

$statement = ""
if ($remaining.Count -gt 0 -and [string]$remaining[0] -eq "--") {
  $remaining = @($remaining | Select-Object -Skip 1)
  if ($remaining.Count -eq 0) {
    [Console]::Error.WriteLine("wecode-db: missing statement after --")
    exit 64
  }
  $statement = ($remaining -join " ")
} elseif ($remaining.Count -gt 0 -and [string]$remaining[0] -eq "--file") {
  if ($remaining.Count -lt 2) {
    [Console]::Error.WriteLine("wecode-db: missing path after --file")
    exit 64
  }
  $statementPath = [string]$remaining[1]
  if (-not (Test-Path $statementPath)) {
    [Console]::Error.WriteLine("wecode-db: unable to read statement file")
    exit 66
  }
  $statement = Get-Content -Raw -LiteralPath $statementPath
} elseif ($remaining.Count -gt 0) {
  Usage
  exit 64
} else {
  [Console]::Error.WriteLine("wecode-db: missing SQL statement")
  exit 64
}

$env:WECODE_DB_PROFILE_ID = $profileId
$env:WECODE_DB_PROFILES_FILE = $profilesFile
$env:WECODE_DB_OUTPUT_JSON = $jsonOutput
$env:WECODE_DB_STATEMENT = $statement
& $helper --wecode-wrapper-helper db-query
exit $LASTEXITCODE
