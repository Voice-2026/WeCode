...
impl CoduxApp {
    ...
    fn terminal_session_input_for_slot(
        ...
        let output = slot
            .pane
            .as_ref()
            .map(|pane| pane.restorable_output_snapshot())
            .or_else(|| {
                self.terminal_manager
                    .restorable_output_snapshot(&terminal_id)
                    .ok()
            });
        let input_bytes = input.as_ref().map(|input| input.bytes).unwrap_or_default();
        let input_history: Vec<TerminalInputSummary> = input
            .map(|input| {
                input
                    .history
                    .into_iter()
...
pub(in crate::app) fn terminal_runtime_summary_from_inputs(
    ...
    let sessions = sessions
        ...
        .map(|session| TerminalRuntimeSessionSummary {
            ...
            ai_launch_mode: session.ai_launch_mode,
        })
        .collect::<Vec<_>>();
