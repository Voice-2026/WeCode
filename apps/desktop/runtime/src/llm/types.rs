...
// The provider-completion option/schema types live in the shared `wecode-llm`
// crate (the agent + memory crate reuse the same genai core). Alias them so the
// desktop's call sites keep their existing names.
pub use wecode_llm::{LlmCompletionOptions as LLMProviderCompletionOptions, LlmJsonSchema as LLMJsonSchema};
