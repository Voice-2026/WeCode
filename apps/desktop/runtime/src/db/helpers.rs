...
pub fn render_db_launch_context(
    ...
    wecode_db_command: Option<String>,
) -> Option<String> {
    render_db_launch_context_from_support_dir(app_support_dir(), project_id, wecode_db_command)
}
...
pub fn render_db_launch_context_from_support_dir(
    ...
    render_db_launch_context_for_profiles(&mut profiles, project_id, wecode_db_command)
}
...
pub(super) fn render_db_launch_context_for_profiles(
    ...
    let wecode_db_command = wecode_db_command
        .and_then(|value| normalized(&value))
        .unwrap_or_else(|| "wecode-db".to_string());
    let lines = vec![
        "WeCode saved database connections for the current root project are available through terminal commands.".to_string(),
        format!(
            "Always run `{wecode_db_command} list` at the time of use to discover the current database profiles as redacted JSON."
        ),
        format!(
            "When a matching saved database profile exists, run `{wecode_db_command} <profile-id> -- '<statement>'`."
        ),
        "Do not grep the repository or inspect WeCode config files to discover saved database connections; use the wrapper list command.".to_string(),
