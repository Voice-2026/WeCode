...
fn show_native_notification_impl(title: &str, body: &str, group: &str) -> Result<(), String> {
    let _ = group;
    let title = native_notification_text(title, "WeCode");
    ...
    notification.appname("WeCode").icon("wecode");
...
fn native_notification_application_identifier() -> String {
    if cfg!(debug_assertions) {
        let installed = notify_rust::get_bundle_identifier_or_default("WeCode");
        if installed != "com.apple.Finder" {
            return installed;
        }
    }
    "com.duxweb.wecode".to_string()
}
