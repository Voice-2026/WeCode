...
//! shared `wecode_git::worktree` engine — the same git2 implementation + managed
//! path convention the desktop uses — so the two hosts can't drift on them.

use serde_json::{Value, json};
use std::path::Path;
use std::process::Command;
use wecode_runtime_core::worktree::{
...
pub fn worktree_list_payload(project_id: &str, project_path: &str) -> Value {
    ...
    let status = wecode_git::wire::status(project_path);
...
/// Create a worktree via the shared `wecode-git` engine — the same git2 backend
/// and managed `.wecode/worktrees/<slug>` path the desktop host uses, so the two
...
pub fn worktree_create(
    ...
    wecode_git::worktree::create_worktree(project_path, branch_name, base_branch).map(|_| ())
}

/// Remove a worktree (and optionally its branch) via the shared `wecode-git`
/// engine.
pub fn worktree_remove(
    ...
    wecode_git::worktree::remove_worktree(project_path, worktree_path, remove_branch)
}
...
pub fn worktree_merge(
    ...
    wecode_git::worktree::merge_worktree(project_path, worktree_path, base_branch, remove_branch)
}
