...
//! the shared `wecode-ai-sessions` op dispatch against its own `ai-usage.sqlite3`
//! — the same table the desktop host uses, so the two never drift.

use serde_json::Value;
use wecode_ai_sessions::AIHistoryService;
...
pub fn ai_session_payload(payload: &Value) -> Value {
    ...
    wecode_ai_sessions::session_op_payload(&service, project_path, payload)
}
