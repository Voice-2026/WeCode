...
cargo build -p wecode-protocol-ffi --target "$TARGET" $PROFILE_FLAG
mkdir -p "$OUT_DIR"
cp "$REPO_ROOT/target/$TARGET/$TARGET_DIR/libwecode_protocol_ffi.a" \
  "$OUT_DIR/libwecode_protocol_ffi.a"
