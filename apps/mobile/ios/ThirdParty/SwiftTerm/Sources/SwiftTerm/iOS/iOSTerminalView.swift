...
open class TerminalView: UIScrollView, UITextInputTraits, UIKeyInput, UIScrollViewDelegate, TerminalDelegate, UIPointerInteractionDelegate {
    ...
    func updateScroller ()
    {
        ...
        let previousMaxOffsetY = max(0, contentSize.height - bounds.height)
        let wasAtBottom = previousOffsetY >= previousMaxOffsetY - max(cellDimension.height, 1)
        let displayBuffer = terminal.displayBuffer
        ...
        let nextOffsetY = wasAtBottom ? maxOffsetY : min(max(previousOffsetY, 0), maxOffsetY)
        contentOffset = CGPoint (x: 0, y: nextOffsetY)
