...
- 通过 Homebrew 安装后会自动提供内置的 `wecode` 命令。

## [2.0.0-rc.11] - 2026-07-16

### 新增

- 新增产品 CLI，支持查询、创建、更新、立即运行、暂停和恢复定时自动任务，并补齐对应的本地控制协议能力。
- 新增 `wecode-control` Skill，便于外部 Agent 通过产品 CLI 查询和控制 WeCode 会话、模型、工作区与自动任务。
- Claude + Kiro 自动任务支持选择模型，新建任务默认使用 Claude + Kiro 与 Opus 4.8。

### 调整

- 自动任务编辑页使用全部可用宽度，并增大任务提示词编辑区域高度。
- Desktop Pull Request 校验和发布仅保留 macOS，Windows 暂不在当前产品范围内。
- 精简 Pull Request CI，只执行编译检查和发布脚本测试；正式构建与打包统一由标签发布工作流执行。

## [2.0.0-rc.10] - 2026-07-14

### 新增

- 新增自动任务管理，支持选择项目、工作区或新建 Worktree，并配置 Agent、会话复用、执行计划和补跑时限。
- 自动任务支持立即运行、暂停、重新启用、编辑和删除，并保留每次运行记录与结果。

### 调整

- 重构自动任务编辑页与通用 UI 组件层，统一卡片、表单、选择器、分段控件和操作栏样式。
- 优化任务列表信息密度、状态展示和编辑布局，任务启停状态更易识别。
- 此预发布版本仅提供 macOS Apple Silicon 安装包。
...
- 无签名测试版本可跳过自动更新清单并正常发布安装包；签名版本仍保持严格校验。

## [2.0.0-rc.7] - 2026-07-13

### 修复

- 修复新仓库未配置 Tauri 更新签名密钥时 GitHub Release 打包失败的问题；存在密钥时仍会强制完成签名。

## [2.0.0-rc.6] - 2026-07-13

### 新增

- 新增 Agent 关注页，将工作中和已完成的 Agent 状态集中展示在独立标签页。
- 支持读取 Kiro App 凭证，并展示多个已绑定微信账号、账号备注、活跃状态和账号切换操作。
- 会话卡片打开时可选择覆盖当前终端或新建终端，右键操作时会保留卡片选中反馈。

### 调整

- 产品名称统一为 WeCode，并完善项目名、界面文案和相关标识。
- 优化终端渲染、快照和局部刷新链路，降低操作与终端输入卡顿。
- 优化终端创建、切换和关闭交互，并将侧栏入口调整为紧凑卡片样式。
- 左右内容栏支持自由拖动调整宽度，缩窄左侧导航栏并统一 macOS 标题栏背景与分隔线。

### 修复

- 修复默认终端在存在多个终端时仍无法关闭的问题。
- 修复终端布局入口失效、悬浮提示缺失及新建终端入口位置异常的问题。
