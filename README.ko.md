...
macOS App에는 `wecode` Product CLI와 `wecode-control` Skill이 함께 포함됩니다. Homebrew 설치는 CLI를 자동으로 노출하며, DMG 설치는 **설정 > Integrations**에서 활성화할 수 있습니다. 같은 화면에서 기존 비관리 사본을 덮어쓰지 않고 감지된 Codex, Claude Code, Kiro에 Skill을 설치할 수 있습니다.

Product CLI는 안정적인 JSON protocol을 통해 외부 Agent가 실행 중인 Desktop을 제어할 수 있게 합니다. 프로젝트와 모델 조회, 세션 생성·재개, prompt 전송, worktree·terminal·예약 자동 작업 관리를 지원합니다.
...
포함된 `wecode-control` Skill은 Codex 등 외부 Agent를 위한 전체 제어 contract를 설명합니다. `wecode integration status --json`으로 설치 상태를 확인할 수 있습니다. 새 자동 작업은 기본적으로 **Claude + Kiro**와 **Opus 4.8**을 사용하며, editor와 CLI 모두에서 모델을 선택할 수 있습니다.
