//! Standalone wecode-gateway binary. Also embeddable via the library `start()`.

use std::path::PathBuf;

use wecode_gateway::{start, GatewayConfig};
...
fn parse_args() -> (Option<PathBuf>, Option<String>, Option<u16>) {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "--help" | "-h" => {
                println!("Usage: wecode-gateway [--config <file>] [--host <host>] [--port <port>]");
                std::process::exit(0);
            }
            _ => {}
        }
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "wecode_gateway=info,tower_http=info".into()),
        )
        .init();
