...
- WeCode service URL normalization for pairing tickets and transport preference rules.
- Transport state/message callbacks.
- Web Tunnel protocol (`/wecode/web-tunnel/1`) for browser preview traffic.
...
Flutter consumes this crate through `wecode-protocol-ffi`, which exposes opaque controller transport handles:
...
Controllers open an independent Iroh custom ALPN, `/wecode/web-tunnel/1`, so page
