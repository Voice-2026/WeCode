use crate::{
    ...
    web_tunnel::{
        WEB_TUNNEL_KIND_TCP_CONNECT, WECODE_WEB_TUNNEL_ALPN, WebTunnelHeader,
...
use wecode_protocol::{REMOTE_TERMINAL_UPLOAD_BLOB, RemoteEnvelope, RemoteTransportKind};

pub const WECODE_REMOTE_ALPN: &[u8] = b"/wecode/remote/1";
const MAX_FRAME_BYTES: usize = 8 * 1024 * 1024;
const MAX_UPLOAD_BYTES: usize = 20 * 1024 * 1024;
const STREAM_KIND_CONTROL: u8 = 0;
const STREAM_KIND_TERMINAL: u8 = 1;
const IROH_RELAY_URL_ENV: &str = "WECODE_IROH_RELAY_URL";
...
const IROH_DISABLE_LAN_MDNS_ENV: &str = "WECODE_IROH_DISABLE_LAN_MDNS";
...
/// incoming `WECODE_REMOTE_ALPN` connections. Holds a `Weak` back-ref so the
/// router (owned by the transport) doesn't keep the transport alive in a cycle.
#[derive(Debug, Clone)]
struct WeCodeRemoteProtocol {
...
#[derive(Debug, Clone)]
struct WeCodeWebTunnelProtocol {
    transport: Weak<RemoteIrohHostTransport>,
}

impl ProtocolHandler for WeCodeRemoteProtocol {
...
impl ProtocolHandler for WeCodeWebTunnelProtocol {
    async fn accept(&self, connection: Connection) -> Result<(), AcceptError> {
        if let Some(transport) = self.transport.upgrade() {
...
impl RemoteIrohHostTransport {
    pub async fn connect(
        ...
        .alpns(vec![
            WECODE_REMOTE_ALPN.to_vec(),
            WECODE_WEB_TUNNEL_ALPN.to_vec(),
        ...
        let router = Router::builder(transport.endpoint.clone())
            .accept(
                WECODE_REMOTE_ALPN,
                WeCodeRemoteProtocol {
                    transport: Arc::downgrade(&transport),
                },
            )
            .accept(
                WECODE_WEB_TUNNEL_ALPN,
                WeCodeWebTunnelProtocol {
                    transport: Arc::downgrade(&transport),
                },
            )
    ...
    fn handle_frame(
        ...
        if raw.kind == wecode_protocol::REMOTE_PAIRING_REQUEST {
...
impl RemoteIrohControllerTransport {
    pub async fn connect(
        ...
        let candidate = config
            ...
            .iter()
            .find(|candidate| {
                candidate.kind == wecode_protocol::REMOTE_TRANSPORT_IROH
        ...
        for attempt in 0..IROH_DIAL_ATTEMPTS {
            ...
            match tokio::time::timeout(
                dial_timeout,
                endpoint.connect(endpoint_addr.clone(), WECODE_REMOTE_ALPN),
            )
            .await
            {
                Ok(Ok(established)) => {
    ...
    async fn open_web_tunnel_connection(&self) -> Result<Connection, String> {
        ...
        self.endpoint
            .connect(remote_id, WECODE_WEB_TUNNEL_ALPN)
            .await
...
pub(crate) fn host_secret_key(config: &RemoteHostTransportConfig) -> Option<SecretKey> {
    ...
    hasher.update(b"wecode-iroh-host-secret-key-v1");
