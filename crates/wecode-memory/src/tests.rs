...
fn temp_support_dir() -> PathBuf {
    let dir = std::env::temp_dir().join(format!("wecode-gpui-memory-test-{}", Uuid::new_v4()));
...
fn enqueue_completed_session_creates_pending_memory_task() {
    let support_dir = temp_support_dir();
    let transcript_dir = std::env::temp_dir().join(format!("wecode-transcript-{}", Uuid::new_v4()));
...
fn enqueue_completed_session_respects_cooldown() {
    ...
    fs::write(
        &transcript,
        "user decided config path is crates/wecode-memory/src/config.rs\nassistant fixed memory config\n",
    )
    .unwrap();
    let service = MemoryService::new(support_dir.clone());
    service.ensure_queue_schema().unwrap();
...
fn enqueue_completed_session_skips_insufficient_growth() {
    ...
    fs::write(
        &transcript,
        "user decided config path crates/wecode-memory/src/config.rs\nassistant fixed memory config bug\n",
    )
    .unwrap();
    let service = MemoryService::new(support_dir.clone());
    let settings = MemorySettings {
...
fn long_transcript_growth_uses_full_file_line_count_not_tail_window() {
    ...
    let base_lines = (0..80)
        .map(|index| format!("log line {index}: fixed config path crates/wecode-memory/src/config.rs"))
        .collect::<Vec<_>>();
