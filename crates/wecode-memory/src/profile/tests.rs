...
fn temp_dir() -> std::path::PathBuf {
    let dir = std::env::temp_dir().join(format!("wecode-profile-test-{}", Uuid::new_v4()));
...
fn project_profile_is_generated_from_repository_files() {
    ...
    fs::write(
        ...
        r#"{"name":"wecode-ui","description":"WeCode desktop shell","scripts":{"dev":"vite","build":"vite build"},"dependencies":{"@tauri-apps/api":"latest","react":"latest"},"devDependencies":{"typescript":"latest","vite":"latest"}}"#,
    )
    .unwrap();
    fs::write(
    ...
    let profile = build_project_profile("project-1", "WeCode", root.to_str().unwrap()).unwrap();

    assert!(profile.content.contains("Project: wecode-ui"));
...
fn project_profile_llm_response_decodes_structured_profile() {
    let decoded = decode_project_profile_llm_response_detailed(
        r#"{"project":"WeCode","overview":"Rust GPUI shell","tech_stack":["Rust","GPUI"],"common_commands":["cargo check"],"top_level_directories":["src"],"detected_modules":["runtime"]}"#,
    )
    .unwrap();

    assert!(decoded.contains("Project: WeCode"));
...
fn project_profile_for_launch_stores_profile() {
    ...
    let profile = service
        .project_profile_for_launch("project-a", "WeCode", root.to_str().unwrap())
        .unwrap();
    let stored = service
