//! Bridge from the memory engine's provider calls to the shared `wecode-llm`
//! crate. The moved engine code calls `llm::complete_with_provider_options`
//! with a [`crate::MemoryProvider`]; this converts it to a
//! [`wecode_llm::LlmProvider`] and runs the shared completion core.
...
pub async fn complete_with_provider_options(
    ...
    wecode_llm::complete(&provider.to_llm_provider(), prompt, system_prompt, options).await
}
