...
pub fn handle_args(args: &[String]) -> Result<bool, String> {
    ...
    if args.first().map(String::as_str) != Some("--wecode-wrapper-helper")
        && env_value("WECODE_WRAPPER_HELPER_ASKPASS") == "1"
    ...
    if command != "--wecode-wrapper-helper" {
        return Ok(false);
    }
...
fn print_claude_memory_context() -> Result<(), String> {
    ...
    let prefix = format!(
        "WeCode memory refresh: the conversation may have been compacted, or this is a new user turn. Re-apply relevant durable memory below. Prefer current user instructions and repository state over stale memory. Memory index file: {path}\n\n"
    );
    let suffix = "\n[WeCode memory refresh truncated]";
...
fn print_ssh_profiles() -> Result<(), String> {
    let path = env_value("WECODE_SSH_PROFILES_FILE");
    let root = read_json_file(&path)
        .ok_or_else(|| "wecode-ssh: failed to read SSH profiles".to_string())?;
    let profiles = ssh_profiles_array(&root)
        .ok_or_else(|| "wecode-ssh: invalid SSH profile file".to_string())?;
...
fn resolve_ssh_profile() -> Result<(String, ResolvedSshProfile), String> {
    let profile_id = env_value("WECODE_SSH_PROFILE_ID").to_ascii_lowercase();
    let path = env_value("WECODE_SSH_PROFILES_FILE");
    let root = read_json_file(&path)
        .ok_or_else(|| "wecode-ssh: failed to read SSH profiles".to_string())?;
    let profiles = ssh_profiles_array(&root)
    ...
    let profile = profiles
        ...
        .ok_or_else(|| "wecode-ssh: SSH profile not found".to_string())?;
    let host = string_field(profile, "host");
    let username = string_field(profile, "username");
    if host.is_empty() || username.is_empty() {
        return Err("wecode-ssh: SSH profile is missing host or username".to_string());
    }
...
fn is_wrapper_helper_exe_name(name: &str) -> bool {
    name.eq_ignore_ascii_case("wecode-wrapper-helper")
}

fn print_ssh_askpass(args: &[String]) -> Result<(), String> {
    let prompt = args.join(" ");
    let password = env_value("WECODE_SSH_PASSWORD");
    let key_passphrase = env_value("WECODE_SSH_KEY_PASSPHRASE");
    let Some(response) = ssh_askpass_response(&prompt, &password, &key_passphrase) else {
        return Err("wecode-ssh: no saved credential matches the SSH prompt".to_string());
    };
    println!("{response}");
    Ok(())
}
...
fn print_db_profiles() -> Result<(), String> {
    let path = env_value("WECODE_DB_PROFILES_FILE");
    let project_id = env_value("WECODE_DB_PROJECT_ID");
    let root = read_json_file(&path)
        .ok_or_else(|| "wecode-db: failed to read database profiles".to_string())?;
    let profiles = db_profiles_array(&root)
        .ok_or_else(|| "wecode-db: invalid database profile file".to_string())?;
    let public_profiles = profiles
        .iter()
...
fn print_db_query() -> Result<(), String> {
    ...
    let output_json = env_value("WECODE_DB_OUTPUT_JSON") == "true";
...
fn ssh_control_path(profile_id: &str) -> Option<String> {
    ...
    let base_dir = env::var("XDG_RUNTIME_DIR")
        ...
        .unwrap_or_else(|| "/tmp".to_string());
    let dir = Path::new(&base_dir).join("wecode-ssh");
...
fn selected_db_profile() -> Result<Value, String> {
    let profile_id = env_value("WECODE_DB_PROFILE_ID");
    let project_id = env_value("WECODE_DB_PROJECT_ID");
    if profile_id.trim().is_empty() {
        return Err("wecode-db: missing database profile id".to_string());
    }
    if project_id.trim().is_empty() {
        return Err("wecode-db: missing WeCode project context".to_string());
    }
    let path = env_value("WECODE_DB_PROFILES_FILE");
    let root = read_json_file(&path)
        .ok_or_else(|| "wecode-db: failed to read database profiles".to_string())?;
    ...
    profiles
        ...
        .cloned()
        .ok_or_else(|| "wecode-db: database profile not found for this project".to_string())
}

fn db_statement() -> Result<String, String> {
    let statement = env_value("WECODE_DB_STATEMENT");
    let statement = statement.trim();
    if statement.is_empty() {
        return Err("wecode-db: missing SQL statement".to_string());
    }
...
fn run_db_query(profile: &Value, statement: &str) -> Result<Value, String> {
    if !statement_allowed(profile, statement) {
        return Err("wecode-db: read-only database profile only allows SELECT/SHOW/WITH/EXPLAIN/PRAGMA statements".to_string());
    }
    ...
    let future = async move {
        ...
        let mut connection = AnyConnection::connect(&url)
            .await
            .map_err(|error| format!("wecode-db: failed to connect: {error}"))?;
        apply_db_session_guard(&mut connection, &engine, read_only).await?;
        let rows = query::<Any>(statement)
            .fetch_all(&mut connection)
            .await
            .map_err(|error| format!("wecode-db: query failed: {error}"))?;
    ...
    F: std::future::Future<Output = Result<Value, String>>,
{
    let runtime = tokio::runtime::Builder::new_current_thread()
        .enable_all()
        .build()
        .map_err(|error| format!("wecode-db: failed to start query runtime: {error}"))?;
    runtime.block_on(async {
        tokio::time::timeout(Duration::from_secs(DB_QUERY_TIMEOUT_SECONDS), future)
            .await
            .map_err(|_| {
                format!("wecode-db: query timed out after {DB_QUERY_TIMEOUT_SECONDS} seconds")
            })?
    })
}
    ...
    match engine {
        "sqlite" | "sqlite3" => {
            query::<Any>("PRAGMA query_only = ON")
                ...
                .await
                .map_err(|error| {
                    format!("wecode-db: failed to enable sqlite read-only guard: {error}")
                })?;
        }
        "postgres" | "postgresql" | "pg" => {
            query::<Any>("SET default_transaction_read_only = on")
                ...
                .await
                .map_err(|error| {
                    format!("wecode-db: failed to enable postgres read-only guard: {error}")
                })?;
        }
        "mysql" | "mariadb" => {
            query::<Any>("SET SESSION TRANSACTION READ ONLY")
                ...
                .await
                .map_err(|error| {
                    format!("wecode-db: failed to enable mysql read-only guard: {error}")
                })?;
        }
        _ => {}
    }
...
fn db_connection_url(profile: &Value) -> Result<String, String> {
    ...
    if database.is_empty() {
        return Err("wecode-db: database name/path cannot be empty".to_string());
    }
    ...
    let username = string_field(profile, "username");
    if host.is_empty() || username.is_empty() {
        return Err("wecode-db: database host and username are required".to_string());
    }
    ...
    let scheme = match engine.as_str() {
        ...
        "mysql" | "mariadb" => "mysql",
        _ => return Err("wecode-db: unsupported database engine".to_string()),
    };
...
mod tests {
    ...
    fn handle_args_rejects_missing_or_unknown_subcommands() {
        assert!(handle_args(&["--wecode-wrapper-helper".to_string()]).is_err());
        assert!(
            handle_args(&[
                "--wecode-wrapper-helper".to_string(),
    ...
    fn askpass_mode_only_answers_from_the_helper_binary_name() {
        assert!(is_wrapper_helper_exe_name("wecode-wrapper-helper"));
        assert!(is_wrapper_helper_exe_name("WECODE-WRAPPER-HELPER"));
        assert!(!is_wrapper_helper_exe_name("wecode"));
        assert!(!is_wrapper_helper_exe_name("wecode-agent"));
    }
    ...
    fn sqlite_path_encoder_preserves_windows_drive_paths_as_absolute() {
        assert_eq!(
            encode_sqlite_path(r"F:\wecode\app.sqlite3"),
            "/F:/wecode/app.sqlite3"
        );
    }

    #[test]
    fn sqlite_db_query_executes_without_exposing_passwords() {
        let dir = std::env::temp_dir().join(format!("wecode-db-query-{}", uuid::Uuid::new_v4()));
