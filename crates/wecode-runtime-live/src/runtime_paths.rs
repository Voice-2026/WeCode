...
pub fn app_support_candidates() -> Vec<PathBuf> {
    ...
    {
        if cfg!(debug_assertions) {
            return vec![home.join("Library/Application Support/WeCode Dev")];
        }
        return vec![home.join("Library/Application Support/WeCode")];
    }
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        if cfg!(debug_assertions) {
            return vec![base.join("WeCode Dev")];
        }
        // Installed layout keeps data in Data beside WeCode.exe; existing %APPDATA% data keeps winning via the probe.
        let mut candidates = Vec::new();
        if let Some(data) = windows_exe_data_dir() {
            candidates.push(data);
        }
        candidates.push(base.join("WeCode"));
        return candidates;
    }
    ...
    #[cfg(all(not(target_os = "macos"), not(target_os = "windows")))]
    {
        ...
        if cfg!(debug_assertions) {
            vec![base.join("WeCode Dev")]
        } else {
            vec![base.join("WeCode")]
        }
    }
}
...
mod tests {
    ...
    fn slug_matches_build_profile() {
        if cfg!(debug_assertions) {
            assert_eq!(app_slug(), "wecode-dev");
            assert!(runtime_temp_dir().ends_with("wecode-dev"));
        } else {
            assert_eq!(app_slug(), "wecode");
            assert!(runtime_temp_dir().ends_with("wecode"));
        }
    }
    ...
    fn support_dir_matches_build_profile() {
        ...
        if cfg!(debug_assertions) {
            ...
            assert!(candidates[0].ends_with("WeCode Dev"));
        } else {
