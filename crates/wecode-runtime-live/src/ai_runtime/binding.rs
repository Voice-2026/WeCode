...
impl AIRuntimeBinding {
    pub fn normalized(mut self) -> Option<Self> {
        ...
        self.provider = normalized_string(self.provider.as_deref());
...
mod tests {
    ...
    fn scan_runtime_bindings_emits_changed_files_only() {
        let dir = std::env::temp_dir().join(format!("wecode-runtime-bindings-{}", Uuid::new_v4()));
    ...
    fn runtime_binding_project_name_falls_back_to_path_basename() {
        let binding = AIRuntimeBinding {
            ...
            project_path: Some("/tmp/wecode-gpui/".to_string()),
        ...
        assert_eq!(binding.project_name, "wecode-gpui");
    }
