...
use wecode_ai_history::agy_db::parse_agy_conversation_db;
...
fn agy_response_state(conversation: &wecode_ai_history::agy_db::AgyConversation) -> Option<String> {
...
fn agy_completed_at(conversation: &wecode_ai_history::agy_db::AgyConversation) -> Option<f64> {
    let last_user_at = conversation.last_user_at.unwrap_or(0.0);
    let last_model_at = conversation.last_model_at.unwrap_or(0.0);
...
mod tests {
    ...
    fn user_after_model_is_responding() {
        let conversation = wecode_ai_history::agy_db::AgyConversation {
