...
impl ClaudeAggregate {
    ...
    /// Only `bypassPermissions` (wecode's `--dangerously-skip-permissions`) silences
...
mod tests {
    ...
    fn missing_bound_id_uses_launch_fresh_cwd_session() {
        let dir = std::env::temp_dir().join(format!(
            "wecode-claude-resume-missing-bound-{}",
            uuid::Uuid::new_v4()
        ));
    ...
    fn existing_bound_id_wins_when_it_is_freshest() {
        let dir = std::env::temp_dir().join(format!(
            "wecode-claude-resume-bound-freshest-{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn bound_latest_does_not_scan_older_fallback_candidate() {
        let dir = std::env::temp_dir().join(format!(
            "wecode-claude-resume-bound-no-scan-{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn newer_resume_session_can_replace_existing_bound_id() {
        let dir = std::env::temp_dir().join(format!(
            "wecode-claude-resume-newer-fallback-{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn launch_bound_rejects_prelaunch_transcript() {
        let dir = std::env::temp_dir().join(format!(
            "wecode-claude-resume-prelaunch-{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn fallback_skips_session_owned_by_other_terminal() {
        let dir = std::env::temp_dir().join(format!(
            "wecode-claude-resume-owned-fallback-{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn prelaunch_unfinished_resume_is_silent_idle() {
        let dir = std::env::temp_dir().join(format!(
            "wecode-claude-resume-stale-open-{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn postlaunch_append_restores_claude_responding() {
        let dir = std::env::temp_dir().join(format!(
            "wecode-claude-resume-postlaunch-{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn live_turn_with_cancel_wording_stays_responding() {
        use std::io::Write;
        let dir =
            std::env::temp_dir().join(format!("wecode-claude-probe-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn local_slash_commands_do_not_start_a_runtime_turn() {
        use std::io::Write;
        let dir =
            std::env::temp_dir().join(format!("wecode-claude-local-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("session.jsonl");
        let mut file = std::fs::File::create(&path).unwrap();
        writeln!(
    ...
    fn compact_summary_rows_do_not_start_a_runtime_turn() {
        use std::io::Write;
        let dir =
            std::env::temp_dir().join(format!("wecode-claude-compact-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("session.jsonl");
        let mut file = std::fs::File::create(&path).unwrap();
        writeln!(
    ...
    fn parses_permission_mode_and_pending_tool_use_from_transcript() {
        use std::io::Write;
        let dir = std::env::temp_dir().join(format!("wecode-claude-wait-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("session.jsonl");
        let mut file = std::fs::File::create(&path).unwrap();
    ...
    fn incremental_cache_waits_for_complete_jsonl_line() {
        use std::io::Write;
        let dir =
            std::env::temp_dir().join(format!("wecode-claude-partial-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("s1.jsonl");
        let first = r#"{"sessionId":"s1","cwd":"/tmp/p","timestamp":"2026-01-01T00:00:00Z","type":"user","message":{"role":"user","content":"run"}}"#;
    ...
    fn incremental_cache_pending_partial_without_growth_does_not_rebuild() {
        use std::io::Write;
        let dir = std::env::temp_dir().join(format!(
            "wecode-claude-pending-no-growth-{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("s1.jsonl");
    ...
    fn incremental_cache_rebuilds_after_truncation() {
        use std::io::Write;
        let dir =
            std::env::temp_dir().join(format!("wecode-claude-truncate-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("s1.jsonl");
    ...
    fn incremental_cache_rebuilds_when_size_is_rewritten_equal() {
        use std::io::Write;
        let dir = std::env::temp_dir().join(format!(
            "wecode-claude-rewrite-same-size-{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("s1.jsonl");
