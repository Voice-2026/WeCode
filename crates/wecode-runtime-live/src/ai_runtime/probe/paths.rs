...
fn agy_db_belongs_to_project(path: &Path, project_path: &str) -> bool {
    wecode_ai_history::agy_db::parse_agy_conversation_db(path)
...
fn agy_db_belongs_to_project_and_started_after(
    ...
    let Some(conversation) = wecode_ai_history::agy_db::parse_agy_conversation_db(path) else {
        return false;
    };
...
mod tests {
    ...
    fn codex_session_id_from_rollout_falls_back_to_session_meta() {
        let dir =
            std::env::temp_dir().join(format!("wecode-codex-rollout-{}", uuid::Uuid::new_v4()));
    ...
    fn kimi_session_from_index_uses_current_agents_main_wire() {
        let share =
            std::env::temp_dir().join(format!("wecode-kimi-index-{}", uuid::Uuid::new_v4()));
    ...
    fn agy_runtime_db_requires_fresh_file_without_resume_id() {
        let dir = std::env::temp_dir().join(format!("wecode-agy-db-{}", uuid::Uuid::new_v4()));
        fs::create_dir_all(&dir).unwrap();
