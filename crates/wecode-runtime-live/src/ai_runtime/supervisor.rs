...
impl AIRuntimeSupervisor {
    ...
    pub fn start(
        ...
        thread::Builder::new()
            .name("wecode-ai-runtime-supervisor".to_string())
            .spawn(move || {
                supervisor_loop(
                    rx,
                    registry,
...
fn start_poll_loop(tx: SyncSender<AIRuntimeSupervisorMessage>) {
    let _ = thread::Builder::new()
        .name("wecode-ai-runtime-poller".to_string())
        .spawn(move || {
            loop {
...
fn start_transcript_monitor_loop(
    ...
    let _ = thread::Builder::new()
        .name("wecode-ai-runtime-transcript-monitor".to_string())
        .spawn(move || {
            loop {
...
fn start_event_dir_watcher(tx: SyncSender<AIRuntimeSupervisorMessage>, runtime_event_dir: PathBuf) {
    let _ = thread::Builder::new()
        .name("wecode-ai-runtime-event-watcher".to_string())
        .spawn(move || {
...
fn start_binding_dir_watcher(tx: SyncSender<AIRuntimeSupervisorMessage>, binding_dir: PathBuf) {
    let _ = thread::Builder::new()
        .name("wecode-ai-runtime-binding-watcher".to_string())
        .spawn(move || {
            use notify::{EventKind, RecursiveMode, Watcher};
...
fn detection_enabled() -> bool {
    !std::env::var("WECODE_AI_RUNTIME_DETECT").is_ok_and(|value| {
        matches!(
...
mod tests {
    ...
    fn supervisor_applies_hook_frames_and_drains_events() {
        ...
        let dir = std::env::temp_dir().join(format!("wecode-supervisor-{}", uuid::Uuid::new_v4()));
        ...
        supervisor
            .submit_frame(
                br#"{"kind":"ai-hook","payload":{"kind":"promptSubmitted","terminalID":"term-1","projectID":"project-1","projectName":"WeCode","projectPath":"/tmp/project","sessionTitle":"Task","tool":"codex","updatedAt":10}}"#
                    .to_vec(),
            )
            .unwrap();
    ...
    fn supervisor_ignores_legacy_agy_hook_frames() {
        ...
        let registry = AIRuntimeRegistry::shared();
        let dir =
            std::env::temp_dir().join(format!("wecode-supervisor-agy-{}", uuid::Uuid::new_v4()));
        let binding_dir = dir.join("bindings");
        supervisor
            .start(Arc::clone(&registry), dir.clone(), binding_dir)
            .unwrap();
        supervisor
            .submit_frame(
                br#"{"kind":"ai-hook","payload":{"kind":"promptSubmitted","terminalID":"term-agy","projectID":"project-1","projectName":"WeCode","projectPath":"/tmp/project","sessionTitle":"Agy","tool":"agy","updatedAt":10}}"#
                    .to_vec(),
            )
            .unwrap();
        std::thread::sleep(std::time::Duration::from_millis(50));
    ...
    fn supervisor_drains_claude_hook_event_files() {
        ...
        let dir = std::env::temp_dir().join(format!(
            "wecode-supervisor-claude-events-{}",
            uuid::Uuid::new_v4()
        ));
        let binding_dir = dir.join("bindings");
        ...
        std::fs::write(
            ...
            format!(
                r#"{{"kind":"ai-hook","payload":{{"kind":"promptSubmitted","terminalID":"term-claude","terminalInstanceID":"instance-claude","projectID":"project-1","projectName":"WeCode","projectPath":"/tmp/project","sessionTitle":"Claude","tool":"claude","aiSessionID":"external-claude","model":"sonnet","updatedAt":{updated_at},"metadata":{{"source":"user-input"}}}}}}"#
            ),
        )
        .unwrap();
    ...
    fn supervisor_applies_runtime_binding_files_without_process_detection() {
        ...
        std::fs::write(
            ...
            format!(
                r#"{{"runtimeBindingId":"instance-1-codex","terminalId":"term-1","terminalInstanceId":"instance-1","tool":"codex","projectId":"project-1","projectName":"WeCode","projectPath":"/tmp/project","sessionTitle":"Codex","launchStartedAt":{started_at},"updatedAt":{started_at}}}"#
            ),
        )
        .unwrap();
    ...
    fn binding_file_event_applies_only_changed_file() {
        ...
        std::fs::write(
            &path,
            r#"{"runtimeBindingId":"instance-1-codex","terminalId":"term-1","terminalInstanceId":"instance-1","tool":"codex","projectId":"project-1","projectName":"WeCode","projectPath":"/tmp/project","sessionTitle":"Codex","launchStartedAt":1000.0,"updatedAt":1000.0}"#,
        )
        .unwrap();
    ...
    fn supervisor_uses_terminal_registry_to_keep_running_session_live() {
        ...
        supervisor
            .submit_frame(
                format!(
                    r#"{{"kind":"ai-hook","payload":{{"kind":"sessionStarted","terminalID":"term-1","terminalInstanceID":"instance-1","projectID":"project-1","projectName":"WeCode","projectPath":"/tmp/project","sessionTitle":"Task","tool":"codex","aiSessionID":"session-1","updatedAt":{started_at}}}}}"#
                )
                .into_bytes(),
            )
            .unwrap();
        supervisor
            .submit_frame(
                format!(
                    r#"{{"kind":"ai-hook","payload":{{"kind":"promptSubmitted","terminalID":"term-1","terminalInstanceID":"instance-1","projectID":"project-1","projectName":"WeCode","projectPath":"/tmp/project","sessionTitle":"Task","tool":"codex","aiSessionID":"session-1","updatedAt":{prompt_at}}}}}"#
                )
                .into_bytes(),
            )
            .unwrap();
    ...
    fn poll_ignores_codewhale_screen_signal_while_idle() {
        ...
        let screen = Arc::new(parking_lot::Mutex::new(
            wecode_terminal_core::HeadlessTerminalScreen::new(80, 24, 100),
        ));
        screen
            .lock()
    ...
    fn process_detection_is_enabled_unless_explicitly_disabled() {
        unsafe {
            std::env::remove_var("WECODE_AI_RUNTIME_DETECT");
        }
        assert!(detection_enabled());
        unsafe {
            std::env::set_var("WECODE_AI_RUNTIME_DETECT", "0");
        }
        assert!(!detection_enabled());
        unsafe {
            std::env::set_var("WECODE_AI_RUNTIME_DETECT", "false");
        }
        assert!(!detection_enabled());
        unsafe {
            std::env::set_var("WECODE_AI_RUNTIME_DETECT", "1");
        }
        assert!(detection_enabled());
        unsafe {
            std::env::remove_var("WECODE_AI_RUNTIME_DETECT");
