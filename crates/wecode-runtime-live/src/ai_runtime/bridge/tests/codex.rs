...
fn codex_wrapper_applies_tool_permissions_and_memory_injection() {
    ...
    let dir = std::env::temp_dir().join(format!("wecode-codex-wrapper-perms-{}", Uuid::new_v4()));
    ...
    assert!(args.contains("# WeCode Environment Directive"));
    assert!(args.contains("wecode-ssh list"));
...
fn mimo_wrapper_uses_managed_xdg_config_for_system_transform() {
    ...
    let dir = std::env::temp_dir().join(format!("wecode-mimo-wrapper-{}", Uuid::new_v4()));
    let bridge = AIRuntimeBridge::with_paths(dir.join("root"), dir.join("temp"), dir.join("home"));
    bridge.stage_assets().unwrap();
...
"#,
    ...
    fs::write(&prompt_file, "WeCode directive").unwrap();
    let search_path = format!("{}:/usr/bin:/bin:/usr/sbin:/sbin", real_bin.display());
...
fn codex_wrapper_reads_tool_permissions_on_each_launch() {
    ...
    let dir = std::env::temp_dir().join(format!(
        "wecode-codex-wrapper-hot-settings-{}",
        Uuid::new_v4()
    ));
    let bridge = AIRuntimeBridge::with_paths(dir.join("root"), dir.join("temp"), dir.join("home"));
    bridge.stage_assets().unwrap();
...
fn codex_wrapper_applies_tool_permissions_when_helper_is_broken() {
    ...
    let dir = std::env::temp_dir().join(format!(
        "wecode-codex-wrapper-helper-broken-{}",
        Uuid::new_v4()
    ));
    let bridge = AIRuntimeBridge::with_paths(dir.join("root"), dir.join("temp"), dir.join("home"));
    bridge.stage_assets().unwrap();
    ...
    let broken_helper = wrapper_dir.join("wecode-wrapper-helper");
    fs::write(
        &broken_helper,
        "#!/bin/sh\necho 'error: Unrecognized option: wecode-wrapper-helper' >&2\nexit 2\n",
    )
    .unwrap();
...
fn wrapper_helper_allowlist_includes_agent_binary() {
    assert!(current_exe_can_act_as_wrapper_helper(Path::new("wecode")));
...
fn codex_wrapper_writes_resume_session_id_to_runtime_binding() {
    ...
    let dir = std::env::temp_dir().join(format!("wecode-codex-wrapper-resume-{}", Uuid::new_v4()));
    let bridge = AIRuntimeBridge::with_paths(dir.join("root"), dir.join("temp"), dir.join("home"));
    bridge.stage_assets().unwrap();
...
fn wrapper_timestamp_handles_comma_decimal_locale() {
    ...
    let dir = std::env::temp_dir().join(format!("wecode-wrapper-comma-locale-{}", Uuid::new_v4()));
    let bridge = AIRuntimeBridge::with_paths(dir.join("root"), dir.join("temp"), dir.join("home"));
    bridge.stage_assets().unwrap();
...
fn hook_event_names_use_millisecond_numeric_prefix() {
    ...
    let dir = std::env::temp_dir().join(format!("wecode-hook-event-timestamp-{}", Uuid::new_v4()));
    let bridge = AIRuntimeBridge::with_paths(dir.join("root"), dir.join("temp"), dir.join("home"));
    bridge.stage_assets().unwrap();
    ...
    let output = Command::new(bridge.managed_hook_script())
        .args(["session-start", "wecode", "claude"])
        .env("DMUX_RUNTIME_OWNER", "wecode")
        .env("DMUX_SESSION_ID", "terminal-1")
        .env("DMUX_SESSION_INSTANCE_ID", "instance-1")
        .env("DMUX_PROJECT_ID", "project-1")
        .env("DMUX_PROJECT_NAME", "Project")
