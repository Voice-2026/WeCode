...
fn zsh_runtime_hook_keeps_wrapper_bin_first_after_user_startup_files() {
    ...
    let dir = std::env::temp_dir().join(format!("wecode-zsh-wrapper-path-{}", Uuid::new_v4()));
...
fn zsh_runtime_hook_restores_wrapper_bin_before_each_prompt() {
    ...
    let dir = std::env::temp_dir().join(format!("wecode-zsh-wrapper-precmd-{}", Uuid::new_v4()));
    let bridge = AIRuntimeBridge::with_paths(dir.join("root"), dir.join("temp"), dir.join("home"));
    bridge.stage_assets().unwrap();
...
#[cfg(not(windows))]
#[test]
#[ignore = "zsh shim smoke test; depends on the local zsh's function-vs-PATH resolution after a mid-session PATH rewrite; run with: cargo test -p wecode-runtime-live -- --ignored zsh_runtime_hook_shims_codex"]
fn zsh_runtime_hook_shims_codex_when_path_is_rewritten_after_prompt() {
    use std::os::unix::fs::PermissionsExt;
    use std::process::Command;

    let dir = std::env::temp_dir().join(format!("wecode-zsh-wrapper-shim-{}", Uuid::new_v4()));
    let bridge = AIRuntimeBridge::with_paths(dir.join("root"), dir.join("temp"), dir.join("home"));
    bridge.stage_assets().unwrap();
...
fn zsh_runtime_hook_suppresses_nested_terminal_integrations_in_user_startup() {
    ...
    let dir = std::env::temp_dir().join(format!(
        "wecode-zsh-terminal-integration-{}",
        Uuid::new_v4()
    ));
    let bridge = AIRuntimeBridge::with_paths(dir.join("root"), dir.join("temp"), dir.join("home"));
    bridge.stage_assets().unwrap();
...
"#,
    ...
    assert!(
        output.status.success(),
        "zsh should load WeCode hook without nested terminal integration, stderr={}",
        String::from_utf8_lossy(&output.stderr)
    );
    let stdout = String::from_utf8_lossy(&output.stdout);
...
fn zsh_runtime_hook_preserves_user_configured_histfile() {
    ...
    let dir = std::env::temp_dir().join(format!("wecode-zsh-histfile-{}", Uuid::new_v4()));
    let bridge = AIRuntimeBridge::with_paths(dir.join("root"), dir.join("temp"), dir.join("home"));
    bridge.stage_assets().unwrap();
