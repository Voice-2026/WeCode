...
fn bridge_stages_runtime_assets_without_installing_hooks() {
    let dir = std::env::temp_dir().join(format!("wecode-ai-bridge-{}", Uuid::new_v4()));
    ...
    #[cfg(windows)]
    {
        ...
        assert!(bridge.wrapper_bin_dir().join("wecode-ssh.ps1").is_file());
        assert!(bridge.wrapper_bin_dir().join("wecode-db.ps1").is_file());
...
fn bridge_start_clears_stale_runtime_bindings_before_scan() {
    let dir = std::env::temp_dir().join(format!("wecode-ai-bridge-bindings-{}", Uuid::new_v4()));
    let bridge = AIRuntimeBridge::with_paths(dir.join("root"), dir.join("temp"), dir.join("home"));
    bridge.stage_assets().unwrap();
...
#[cfg(not(windows))]
#[test]
fn tool_wrapper_keeps_wecode_ssh_available_to_ai_cli() {
    use std::os::unix::fs::PermissionsExt;
    use std::process::Command;

    let dir = std::env::temp_dir().join(format!("wecode-ai-wrapper-path-{}", Uuid::new_v4()));
    let bridge = AIRuntimeBridge::with_paths(dir.join("root"), dir.join("temp"), dir.join("home"));
    bridge.stage_assets().unwrap();
    ...
    fs::write(
        &fake_codex,
        "#!/bin/sh\ncommand -v wecode-ssh >/dev/null || exit 42\n",
    )
    .unwrap();
    ...
    assert!(
        ...
        "wrapper should expose wecode-ssh to AI CLI, stderr={}",
