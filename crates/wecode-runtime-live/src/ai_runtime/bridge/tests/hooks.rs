...
fn claude_wrapper_applies_driver_memory_prompt_injection() {
    ...
    let dir = std::env::temp_dir().join(format!("wecode-claude-wrapper-memory-{}", Uuid::new_v4()));
...
#[test]
fn bridge_prepare_strips_wecode_hooks_and_keeps_user_hooks() {
    let dir = std::env::temp_dir().join(format!("wecode-ai-bridge-file-{}", Uuid::new_v4()));
            ...
            "hooks": {
                "UserPromptSubmit": [{
                    ...
                    "hooks": [
                        {
                            "type": "command",
                            "command": "'/old/dmux-ai-state.sh' 'prompt-submit' 'wecode' 'claude'"
                        },
    ...
    .unwrap();
    // The runtime is non-intrusive: prepare() STRIPS wecode hooks, never installs.
    ...
    // The user's own settings and hook survive; only wecode's entry is gone.
    assert_eq!(
...
fn codewhale_wrapper_applies_configured_model_and_resume_session() {
    ...
    let dir = std::env::temp_dir().join(format!("wecode-codewhale-wrapper-{}", Uuid::new_v4()));
    let bridge = AIRuntimeBridge::with_paths(dir.join("root"), dir.join("temp"), dir.join("home"));
    bridge.stage_assets().unwrap();
...
fn kimi_wrapper_applies_configured_model_and_memory_agent_file() {
    ...
    let dir = std::env::temp_dir().join(format!("wecode-kimi-wrapper-{}", Uuid::new_v4()));
    let bridge = AIRuntimeBridge::with_paths(dir.join("root"), dir.join("temp"), dir.join("home"));
    bridge.stage_assets().unwrap();
...
fn kiro_cli_wrapper_writes_runtime_binding_and_model_without_permission_args() {
    ...
    let dir = std::env::temp_dir().join(format!("wecode-kiro-wrapper-{}", Uuid::new_v4()));
    let bridge = AIRuntimeBridge::with_paths(dir.join("root"), dir.join("temp"), dir.join("home"));
    bridge.stage_assets().unwrap();
...
fn codewhale_hook_writes_runtime_event() {
    ...
    let dir = std::env::temp_dir().join(format!("wecode-codewhale-event-{}", Uuid::new_v4()));
    let bridge = AIRuntimeBridge::with_paths(dir.join("root"), dir.join("temp"), dir.join("home"));
    bridge.stage_assets().unwrap();
...
fn codewhale_hook_without_payload_does_not_block() {
    ...
    let dir = std::env::temp_dir().join(format!("wecode-codewhale-empty-{}", Uuid::new_v4()));
    let bridge = AIRuntimeBridge::with_paths(dir.join("root"), dir.join("temp"), dir.join("home"));
    bridge.stage_assets().unwrap();
    let events = dir.join("events");
    fs::create_dir_all(&events).unwrap();
...
fn codewhale_turn_end_hook_writes_interrupted_lifecycle_event() {
    ...
    let dir = std::env::temp_dir().join(format!("wecode-codewhale-turn-end-{}", Uuid::new_v4()));
    let bridge = AIRuntimeBridge::with_paths(dir.join("root"), dir.join("temp"), dir.join("home"));
    bridge.stage_assets().unwrap();
    let events = dir.join("events");
    fs::create_dir_all(&events).unwrap();
