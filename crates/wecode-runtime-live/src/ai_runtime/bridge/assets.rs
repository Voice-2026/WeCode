...
impl AIRuntimeBridge {
    pub fn stage_assets(&self) -> Result<(), String> {
        ...
        stage_runtime_asset(
            ...
            &wrapper_dir.join("wecode-ssh-expect.exp"),
            true,
        )?;
        #[cfg(windows)]
        {
            ...
            stage_runtime_asset(
                ...
                &wrapper_dir.join("wecode-ssh.ps1"),
                false,
            )?;
            stage_runtime_asset(
                "scripts/wrappers/wecode-db.ps1",
                &wrapper_dir.join("wecode-db.ps1"),
                false,
            )?;
        }
        stage_runtime_dir(
        ...
        let mut bin_names = ai_runtime_tool_drivers()
            ...
            .collect::<Vec<_>>();
        bin_names.push("wecode-ssh");
        bin_names.push("wecode-db");
    ...
    fn stage_wrapper_helper(&self, wrapper_dir: &Path) -> Result<(), String> {
        let helper_path = wrapper_dir.join("wecode-wrapper-helper");
        #[cfg(test)]
        {
    ...
    #[cfg(windows)]
    fn stage_wrapper_helper(&self, wrapper_dir: &Path) -> Result<(), String> {
        let helper_path = wrapper_dir.join("wecode-wrapper-helper.exe");
        // Old versions staged the GUI desktop exe under the extensionless
        // name; the PS wrappers must never pick that up again.
        let _ = fs::remove_file(wrapper_dir.join("wecode-wrapper-helper"));
        #[cfg(test)]
        {
...
pub(super) fn current_exe_can_act_as_wrapper_helper(path: &Path) -> bool {
    path.file_name()
        ...
        .is_some_and(|name| matches!(name, "wecode" | "WeCode" | "WeCode Dev" | "wecode-agent"))
}
...
fn windows_console_exe_can_act_as_wrapper_helper(path: &Path) -> bool {
    path.file_stem()
        .and_then(|name| name.to_str())
        .is_some_and(|name| name.eq_ignore_ascii_case("wecode-agent"))
}
...
fn codewhale_lifecycle_config_toml(
    ...
    for hook in hooks {
        ...
        content.push_str(&format!(
            "name = {}\n",
            toml_string(&format!("wecode-{tool}-{}", hook.action))?
        ));
