...
mod tests {
    ...
    fn opencode_hook_config_status_matches_embedded_runtime_assets() {
        let home = std::env::temp_dir().join(format!("wecode-opencode-hooks-{}", Uuid::new_v4()));
    ...
    fn hook_config_status_reports_missing_codewhale_hooks() {
        let root = std::env::temp_dir().join(format!("wecode-codewhale-hooks-{}", Uuid::new_v4()));
...
[[hooks.hooks]]
name = "wecode-codewhale-turn-end"
event = "turn_end"
