...
const MANAGED_NAME_PREFIX: &str = "wecode-codewhale-";

pub(in crate::ai_runtime::hooks) fn codewhale_config_path_in(home_dir: &Path) -> PathBuf {
    home_dir.join(".codewhale").join("config.toml")
}

/// Strip wecode-managed codewhale hook blocks, leaving the user's config intact.
...
mod tests {
    ...
    fn uninstall_codewhale_strips_managed_hooks_and_keeps_user_config() {
        let home = std::env::temp_dir().join(format!("wecode-codewhale-{}", Uuid::new_v4()));
...
[[hooks.hooks]]
name = "wecode-codewhale-message-submit"
event = "message_submit"
command = "'/old/dmux-ai-state.sh' 'codewhale-message-submit' 'wecode' 'codewhale'"
...
"#,
        ...
        // wecode-managed block is removed.
