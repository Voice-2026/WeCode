...
const MANAGED_NAME_PREFIX: &str = "wecode-kimi-";

pub(in crate::ai_runtime::hooks) fn kimi_config_path_in(home_dir: &Path) -> PathBuf {
    home_dir.join(".kimi-code").join("config.toml")
}

/// Strip wecode-managed kimi hook blocks, leaving the user's config intact. Never
...
mod tests {
    ...
    fn uninstall_kimi_strips_managed_hooks_and_keeps_user_config() {
        let home = std::env::temp_dir().join(format!("wecode-kimi-{}", Uuid::new_v4()));
...
[[hooks]]
name = "wecode-kimi-prompt-submit"
event = "UserPromptSubmit"
command = "'/old/dmux-ai-state.sh' 'prompt-submit' 'wecode' 'kimi'"
"#,
        )
        .unwrap();
        ...
        // The user's model and their own hook survive; the wecode-managed block is
        // removed.
