...
/// Remove the wecode-managed hook trust blocks from `config.toml` so the dead
...
/// flag untouched (inert CLI toggles, not wecode hooks).
...
mod tests {
    ...
    fn uninstall_codex_config_strips_managed_trust_blocks_and_keeps_user_entries() {
        let dir =
            std::env::temp_dir().join(format!("wecode-codex-uninstall-{}", uuid::Uuid::new_v4()));
        ...
        // wecode-managed trust block removed; the user's custom block, profile and
