...
mod tests {
    ...
    fn recognizes_only_matching_owner_for_shell_hooks() {
        let hook = json!({
            "command": "'/tmp/dmux-ai-state.sh' 'codex-stop' 'wecode-dev' 'codex'"
        });

        assert!(is_managed_hook(&hook, "codex-stop", "wecode-dev", "codex"));
        assert!(!is_managed_hook(&hook, "codex-stop", "wecode", "codex"));
    }

    #[test]
    fn recognizes_windows_hooks_for_cross_platform_cleanup() {
        let current = json!({
            "command": "powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File 'C:\\WeCode\\dmux-ai-state.ps1' 'codex-session-start' 'wecode' 'codex'"
        });
        let hook = json!({
            "command": "cmd /d /c call \"C:\\WeCode\\dmux-ai-state.cmd\" \"codex-session-start\" \"wecode\" \"codex\""
        });
