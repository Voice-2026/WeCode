...
mod tests {
    ...
    fn drains_runtime_event_files_and_removes_them() {
        let dir = std::env::temp_dir().join(format!("wecode-event-drain-{}", Uuid::new_v4()));
    ...
    fn clears_runtime_event_files_without_touching_other_files() {
        let dir = std::env::temp_dir().join(format!("wecode-event-clear-{}", Uuid::new_v4()));
        fs::create_dir_all(&dir).unwrap();
        fs::write(dir.join("one.json"), br#"{"kind":"ai-hook"}"#).unwrap();
    ...
    fn drains_runtime_event_files_in_file_name_order() {
        let dir = std::env::temp_dir().join(format!("wecode-event-order-{}", Uuid::new_v4()));
        fs::create_dir_all(&dir).unwrap();
