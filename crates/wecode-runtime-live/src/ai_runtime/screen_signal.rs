...
use wecode_terminal_core::TerminalScreenSnapshot;
...
/// prompt. wecode already owns every PTY, so one detector covers all tools.
...
mod tests {
    ...
    fn screen_text_rebuilds_unstyled_space_gaps_from_columns() {
        let mut screen = wecode_terminal_core::HeadlessTerminalScreen::new(80, 24, 100);
