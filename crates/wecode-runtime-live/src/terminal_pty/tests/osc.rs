...
fn terminal_progress_osc_emits_working_status_without_session_mutation() {
    let dir =
        std::env::temp_dir().join(format!("wecode-terminal-progress-start-{}", Uuid::new_v4()));
...
fn terminal_progress_osc_emits_completed_status() {
    let dir =
        std::env::temp_dir().join(format!("wecode-terminal-progress-idle-{}", Uuid::new_v4()));
    let bridge = Arc::new(AIRuntimeBridge::with_paths(
        dir.join("root"),
        dir.join("temp"),
        dir.join("home"),
...
fn terminal_notification_osc_emits_waiting_status() {
    let dir = std::env::temp_dir().join(format!(
        "wecode-terminal-notification-waiting-{}",
        Uuid::new_v4()
    ));
    let bridge = Arc::new(AIRuntimeBridge::with_paths(
        dir.join("root"),
...
fn terminal_notification_osc_survives_chunk_split() {
    let dir = std::env::temp_dir().join(format!("wecode-terminal-osc-split-{}", Uuid::new_v4()));
    let bridge = Arc::new(AIRuntimeBridge::with_paths(
        dir.join("root"),
        dir.join("temp"),
        dir.join("home"),
...
fn oversized_unterminated_osc_is_dropped_and_parser_recovers() {
    let dir =
        std::env::temp_dir().join(format!("wecode-terminal-osc-oversized-{}", Uuid::new_v4()));
    let bridge = Arc::new(AIRuntimeBridge::with_paths(
        dir.join("root"),
        dir.join("temp"),
        dir.join("home"),
...
fn terminal_output_refreshes_kiro_screen_signal_without_poll() {
    let dir = std::env::temp_dir().join(format!(
        "wecode-kiro-terminal-screen-signal-{}",
        Uuid::new_v4()
    ));
    let bridge = Arc::new(AIRuntimeBridge::with_paths(
        dir.join("root"),
