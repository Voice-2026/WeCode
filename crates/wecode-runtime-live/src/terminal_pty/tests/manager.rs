...
#[cfg(unix)]
#[test]
fn terminal_io_stream_orders_input_and_output_and_marks_wechat_origin() {
    let manager = TerminalManager::new();
    let emit: EventSink = Arc::new(|_| true);
    let config = TerminalPtyConfig {
        terminal_id: Some(format!("test-terminal-io-{}", Uuid::new_v4())),
        shell: Some("/bin/cat".to_string()),
        cols: Some(80),
        rows: Some(24),
        scrollback_lines: Some(100),
    ...
    let (session, _) = manager
        ...
        .expect("terminal should start");
    let io_rx = manager
        .subscribe_io(session.id())
        .expect("I/O subscription should start");

    manager
        .write(session.id(), b"secret-password\n")
        .expect("desktop input should succeed");
    let input = io_rx
        .recv_timeout(Duration::from_secs(2))
        .expect("input event should arrive");
    assert_eq!(input.direction, TerminalIoDirection::Input);
    assert_eq!(input.origin, TerminalIoOrigin::Local);
    assert_eq!(input.bytes, b"secret-password\n");
    let output = io_rx
        .recv_timeout(Duration::from_secs(2))
        .expect("output event should arrive");
    assert_eq!(output.direction, TerminalIoDirection::Output);
    assert_eq!(output.origin, TerminalIoOrigin::Pty);
    assert!(output.sequence > input.sequence);
    assert!(String::from_utf8_lossy(&output.bytes).contains("secret-password"));

    while io_rx.try_recv().is_ok() {}
    manager
        .write_from_wechat(session.id(), b"wechat-origin\n")
        .expect("WeChat input should succeed");
    let wechat_input = io_rx
        .recv_timeout(Duration::from_secs(2))
        .expect("WeChat input event should arrive");
    assert_eq!(wechat_input.direction, TerminalIoDirection::Input);
    assert_eq!(wechat_input.origin, TerminalIoOrigin::WeChat);
    let deadline = Instant::now() + Duration::from_secs(2);
    let mut echoed = String::new();
    while Instant::now() < deadline && !echoed.contains("wechat-origin") {
        match io_rx.recv_timeout(Duration::from_millis(100)) {
            Ok(event) => {
                assert_eq!(event.direction, TerminalIoDirection::Output);
                assert_eq!(event.origin, TerminalIoOrigin::Pty);
                echoed.push_str(&String::from_utf8_lossy(&event.bytes));
            }
            Err(flume::RecvTimeoutError::Timeout) => {}
            Err(flume::RecvTimeoutError::Disconnected) => break,
        }
    }
    assert!(echoed.contains("wechat-origin"));
...
fn terminal_manager_replaces_same_terminal_id_when_identity_changes() {
    ...
    let first_cwd = std::env::temp_dir().join(format!("wecode-pty-first-{}", Uuid::new_v4()));
    let second_cwd = std::env::temp_dir().join(format!("wecode-pty-second-{}", Uuid::new_v4()));
...
fn terminal_manager_uses_context_session_cwd_for_identity() {
    ...
    let project_cwd = std::env::temp_dir().join(format!("wecode-project-{}", Uuid::new_v4()));
    let worktree_cwd = std::env::temp_dir().join(format!("wecode-worktree-{}", Uuid::new_v4()));
...
fn terminal_manager_registers_ai_runtime_terminal_lifecycle() {
    let dir = std::env::temp_dir().join(format!("wecode-terminal-bridge-{}", Uuid::new_v4()));
