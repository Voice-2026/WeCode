...
#[test]
fn terminal_environment_injects_wecode_runtime_context() {
    let temp =
        std::env::temp_dir().join(format!("wecode-terminal-runtime-root-{}", Uuid::new_v4()));
    ...
    let context = TerminalLaunchContext {
        ...
        session_cwd: Some(PathBuf::from("/workspace/wecode")),
        session_instance_id: Some("session-instance-1".to_string()),
        tool_permissions_file: Some(PathBuf::from("/tmp/wecode/tool-permissions.json")),
        memory_workspace_root: Some(PathBuf::from("/tmp/wecode/memory-workspaces/project-1")),
        memory_prompt_file: Some(PathBuf::from(
            "/tmp/wecode/memory-workspaces/project-1/memory-prompt.txt",
        )),
        memory_index_file: Some(PathBuf::from(
            "/tmp/wecode/memory-workspaces/project-1/MEMORY.md",
        )),
        host_device_id: None,
    };
    let env = terminal_environment(
        "/bin/zsh",
        Some("/workspace/wecode"),
        "gpui-term-1",
        &context.to_config(),
        Some(&context),
    );
    ...
    assert_eq!(
        env.get("WECODE_TERMINAL_ID").map(String::as_str),
        Some("gpui-term-1")
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        Some("/tmp/wecode/memory-workspaces/project-1/MEMORY.md")
    );
    assert_eq!(
    ...
    assert_eq!(
        env.get("WECODE_SSH_PROFILES_FILE").map(String::as_str),
        Some("/support/WeCode/ssh_profiles.json")
    );
    assert_eq!(
        env.get("WECODE_DB_PROFILES_FILE").map(String::as_str),
        Some("/support/WeCode/db_profiles.json")
    );
    assert_eq!(
        env.get("WECODE_DB_PROJECT_ID").map(String::as_str),
        Some("project-1")
    );
    assert_eq!(
...
fn terminal_environment_treats_named_zsh_wrapper_as_zsh() {
    let temp = std::env::temp_dir().join(format!(
        "wecode-terminal-runtime-root-named-zsh-{}",
        Uuid::new_v4()
    ));
    let runtime_root = temp.join("runtime-root");
    fs::create_dir_all(runtime_root.join("scripts/shell-hooks/zsh")).unwrap();
...
fn terminal_environment_does_not_override_zdotdir_when_runtime_zsh_hook_is_incomplete() {
    let temp = std::env::temp_dir().join(format!(
        "wecode-terminal-runtime-root-missing-hook-{}",
        Uuid::new_v4()
    ));
    let runtime_root = temp.join("runtime-root");
    fs::create_dir_all(runtime_root.join("scripts/shell-hooks/zsh")).unwrap();
