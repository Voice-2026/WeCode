...
// Debug lever: WECODE_TERMINAL_CAPTURE=<dir|1> appends each session's raw PTY
...
fn raw_capture_file(session_id: &str) -> Option<RawCapture> {
    let value = std::env::var("WECODE_TERMINAL_CAPTURE").ok()?;
    ...
    let dir = if candidate.is_dir() {
        ...
        std::env::temp_dir().join("wecode-terminal-capture")
    };
