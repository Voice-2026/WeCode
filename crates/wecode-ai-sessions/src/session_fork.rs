...
impl ForkPromptBuilder {
    ...
    fn prompt_text(&mut self) -> String {
        ...
        push_limited(
            ...
            &format!(
                "# Continue Cleaned AI Session\n\n\
                 You are continuing an AI coding session in WeCode with {}.\n\
                 WeCode injects its project memory, user memory, SSH command context, and tool permissions separately at launch. Do not ask the user to paste WeCode memory, and do not duplicate memory content in this prompt.\n\n",
                self.request.target_tool.display_name()
            ),
        );
        ...
        if self.snippets.is_empty() {
            push_limited(
                &mut text,
                "No readable recent transcript text was available. Continue using the project files and WeCode-injected memory.\n",
            );
        } else {
...
fn sanitize_text(text: &str, omitted_items: &mut usize) -> String {
    let text = strip_wecode_injected_context(text, omitted_items);
...
fn strip_wecode_injected_context(text: &str, omitted_items: &mut usize) -> String {
    let markers = [
    ...
    *omitted_items += 1;
    ...
    for line in text.lines() {
        let trimmed = line.trim();
        if trimmed == "# WeCode Environment Directive"
            || trimmed == "# WeCode Memory"
        ...
        if skipping
            && trimmed.starts_with("# ")
            && trimmed != "# WeCode Environment Directive"
            && trimmed != "# WeCode Memory"
        {
            skipping = false;
        }
    ...
    if cleaned.is_empty() {
        "[omitted WeCode injected memory context]".to_string()
    } else {
        format!("[omitted WeCode injected memory context]\n{cleaned}")
    }
}
...
mod tests {
    ...
    #[test]
    fn fork_prompt_does_not_duplicate_wecode_memory() {
        ...
        assert!(prompt.contains("WeCode injects its project memory"));
        assert!(!prompt.contains("## Global Prompt"));
        assert!(!prompt.contains("\n# WeCode Memory\n"));
        assert!(prompt.contains("[omitted WeCode injected memory context]"));
    ...
    fn audit_local_existing_session_forks() {
        ...
        let support_dirs = [
            PathBuf::from(&home).join("Library/Application Support/WeCode Dev"),
            PathBuf::from(&home).join("Library/Application Support/WeCode"),
        ];
        let cases = [
            (
                "codex",
    ...
    fn temp_support_dir(name: &str) -> PathBuf {
        let path =
            std::env::temp_dir().join(format!("wecode-ai-session-fork-{name}-{}", Uuid::new_v4()));
