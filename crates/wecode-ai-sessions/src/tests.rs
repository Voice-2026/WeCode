...
fn rename_and_remove_project_sessions_preserve_summary_totals() {
    ...
    let project_path = "/tmp/wecode-gpui";
...
fn project_summary_includes_tauri_ai_panel_aggregates() {
    ...
    let summary = AIHistoryService::new(support_dir.clone()).project_summary("/tmp/wecode-gpui");
...
fn global_summary_aggregates_projects_and_recent_sessions() {
    ...
    assert_eq!(summary.project_totals[0].project_path, "/tmp/wecode-gpui");
...
fn temp_support_dir(label: &str) -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let dir = std::env::temp_dir().join(format!("wecode-gpui-ai-history-{label}-{nanos}"));
