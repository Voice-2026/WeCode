...
impl LocalPtySession {
    pub fn spawn(
        ...
        let project_name = config
            ...
            .clone()
            .filter(|value| !value.trim().is_empty())
            .unwrap_or_else(|| "WeCode".to_string());
...
fn resilient_cwd_command(shell: &str, cwd: &str, command: Option<&str>) -> String {
    ...
    let mut script = format!(
        "__wecode_target={}; __wecode_wait_logged=0; \
         __wecode_wait_count=0; \
         until cd \"$__wecode_target\" 2>/dev/null; do \
         if [ \"$__wecode_wait_logged\" = 0 ]; then \
         printf '\\r\\nWeCode: waiting for working directory to return: %s\\r\\n' \"$__wecode_target\"; \
         __wecode_wait_logged=1; fi; \
         __wecode_wait_count=$((__wecode_wait_count + 1)); \
         if [ \"$__wecode_wait_count\" -ge 30 ]; then \
         printf 'WeCode: working directory did not return; starting in HOME.\\r\\n'; \
         cd \"$HOME\" 2>/dev/null || cd /; \
         unset __wecode_target __wecode_wait_logged __wecode_wait_count; \
         {}; fi; sleep 2; done; \
         if [ \"$__wecode_wait_logged\" = 1 ]; then \
         printf 'WeCode: working directory is available again.\\r\\n'; fi; \
         unset __wecode_target __wecode_wait_logged __wecode_wait_count; ",
        shell_quote(cwd),
        fallback_shell,
    );
...
fn resilient_cwd_command(_shell: &str, cwd: &str, command: Option<&str>) -> String {
    ...
    format!(
        "$wecodeTarget = {}; $wecodeWaitCount = 0; while (-not (Test-Path -LiteralPath $wecodeTarget -PathType Container)) {{ Write-Host \"WeCode: waiting for working directory to return: $wecodeTarget\"; Start-Sleep -Seconds 2; $wecodeWaitCount += 1; if ($wecodeWaitCount -ge 30) {{ Write-Host \"WeCode: working directory did not return; starting in HOME.\"; Set-Location -LiteralPath $HOME; powershell.exe -NoLogo -NoExit; exit }} }}; Set-Location -LiteralPath $wecodeTarget; {command}",
...
mod tests {
    ...
    fn local_driver_captures_command_output() {
        ...
        let session = driver
            .create(
                TerminalLaunchConfig {
                    command: Some("printf wecode-pty-test".to_string()),
                    ..Default::default()
                },
                Box::new(|_| true),
            )
        ...
        while std::time::Instant::now() < deadline {
            if session.snapshot().contains("wecode-pty-test") {
                let _ = session.kill();
                return;
            }
        ...
        let _ = session.kill();
        assert!(
            snapshot.contains("wecode-pty-test"),
    ...
    fn spawn_waits_for_missing_cwd_to_return() {
        let cwd = std::env::temp_dir().join(format!("wecode-pty-cwd-{}", Uuid::new_v4()));
        ...
        let mut process = spawn_local_pty(LocalPtySpawnConfig {
            ...
            initial_command: Some("pwd; printf wecode-recovered; exit".to_string()),
            cols: 80,
            rows: 24,
            env: HashMap::from([(
                "HOME".to_string(),
        ...
        while std::time::Instant::now() < deadline {
            match process.reader.read(&mut buffer) {
                ...
                Ok(count) => {
                    ...
                    if output.contains("wecode-recovered") {
    ...
    fn resilient_cwd_command_has_home_fallback() {
        ...
        assert!(script.contains("__wecode_wait_count"));
