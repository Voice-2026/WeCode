...
//! operations separately — the desktop via git2 under a `.wecode/worktrees/<slug>`
...
/// `.wecode/worktrees/<slug>` path, and return the created directory.
...
fn repo_signature(repo: &GitRepository) -> Result<git2::Signature<'_>, String> {
    repo.signature()
        .or_else(|_| git2::Signature::now("WeCode", "wecode@example.local"))
        .map_err(|error| error.message().to_string())
}
