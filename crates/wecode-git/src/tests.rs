...
mod tests {
    ...
    fn reads_modified_and_untracked_file_diffs() {
        ...
        config
            ...
            .expect("email");
        config.set_str("user.name", "WeCode").expect("name");
    ...
    fn git_watcher_path_set_keeps_other_worktrees_when_one_is_removed() {
        let mut paths = HashSet::from([
            "/repo/app".to_string(),
            "/repo/app/.wecode/worktrees/task-a".to_string(),
        ]);

        let empty = remove_watched_project_path(
            &mut paths,
            &normalized_path_key(Path::new("/repo/app/.wecode/worktrees/task-a")),
        );
    ...
    fn cancellable_git_commands_pass_cancel_token_to_git2_operations() {
        ...
        let repository = GitRepository::open(&repo).expect("open repo");
        repository
            .remote("origin", "https://example.invalid/wecode.git")
            .expect("remote");
    ...
    fn stash_tag_and_rename_branch_round_trip() {
        ...
        let repository = GitRepository::open(&repo).expect("open repo");
        let mut config = repository.config().expect("config");
        config.set_str("user.email", "wecode@example.test").expect("email");
        config.set_str("user.name", "WeCode").expect("name");
    ...
    fn temp_dir(label: &str) -> PathBuf {
        let nanos = SystemTime::now()
            ...
            .expect("clock")
            .as_nanos();
        let path = std::env::temp_dir().join(format!("wecode-gpui-{label}-{nanos}"));
