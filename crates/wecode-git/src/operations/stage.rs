    ...
    if paths.is_empty() {
        index
            .add_all(
                ...
                Some(&mut |path, _| {
                    if is_wecode_managed_memory_entrypoint_path(repo, path) {
                        1
                    } else {
                        0
                    }
        ...
        remove_wecode_managed_memory_entrypoint_from_index(repo, &mut index);
    } else {
            ...
            if target.is_dir() {
                ...
                index
                    .add_all(
                        ...
                        git2::IndexAddOption::DEFAULT,
                        Some(&mut |entry, _| {
                            if is_wecode_managed_memory_entrypoint_path(repo, entry) {
                                1
                            } else {
                                0
                            }
