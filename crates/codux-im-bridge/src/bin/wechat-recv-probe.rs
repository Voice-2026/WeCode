...
//!
//! Run: CODUX_WECHAT_CREDS=~/Library/.../credentials.json cargo run --bin wechat-recv-probe

use codux_im_bridge::wechat::{ILinkClient, WeChatCredentials};

#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let path = std::env::var("CODUX_WECHAT_CREDS")?;
            ...
            Ok(resp) => {
                ...
                for msg in &resp.msgs {
                    println!(
                        ...
                        msg.from_user_id,
                        msg.item_list.len(),
                        codux_im_bridge::wechat::message_text(msg)
                    );
