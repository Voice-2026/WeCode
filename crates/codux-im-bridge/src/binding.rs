...
//!
//! A *binding* maps one chat peer (a WeChat `from_user_id`) to one Codux
...
/// One chat peer bound to a Codux terminal session.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ChatBinding {
    /// The chat peer id (WeChat `from_user_id`).
    pub chat_id: String,
    /// The Codux terminal session id this peer drives.
