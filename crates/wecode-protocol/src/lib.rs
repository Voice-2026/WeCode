...
/// runs the wecode-memory engine against its own memory store. Ops: `summary`,
...
/// runs the wecode-ai-sessions engine against its own history. Ops: `detail`,
/// `rename`, `remove`, `fork`.
...
/// One transport entry for a pairing QR / `wecode://pair?payload=` URL. Encodes
...
/// embedded in the pairing QR and the `wecode://pair?payload=` URL. Shared by the
