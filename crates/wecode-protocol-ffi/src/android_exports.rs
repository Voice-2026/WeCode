...
#[unsafe(no_mangle)]
pub unsafe extern "system" fn Java_com_wecode_wecode_1protocol_1ffi_WeCodeProtocolFfiPlugin_initAndroidContext(
    mut env: JNIEnv,
    _class: JClass,
    context: JObject,
) -> jboolean {
...
fn init_android_context(env: &mut JNIEnv, context: JObject) -> Result<(), String> {
    ...
    unsafe {
        wecode_remote_transport::install_android_jni_context(java_vm, application_context_ptr)?;
    }
