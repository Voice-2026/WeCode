...
fn normalized_history_title_candidate(value: &str) -> Option<String> {
    strip_wecode_launch_context(value).and_then(|value| normalized_string(&value))
}

fn strip_wecode_launch_context(value: &str) -> Option<String> {
    ...
    if let Some(index) = last_wecode_context_close_index(value) {
        return normalized_string(&value[index..]);
    }

    let trimmed = value.trim_start();
    if is_wecode_injected_title_prefix(trimmed) {
        return None;
    }
...
fn last_wecode_context_close_index(value: &str) -> Option<usize> {
    [
...
fn is_wecode_injected_title_prefix(value: &str) -> bool {
    value.starts_with("# AGENTS.md")
        || value.starts_with("# Continue Cleaned AI Session")
        || value.starts_with("# WeCode Memory")
